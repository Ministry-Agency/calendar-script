...
class UnifiedCalendarManager {
    ...
    updateCalendar() {
        try {
            ...
            // Сохраняем существующие обработчики событий
            const existingHandlers = new Map();
            document.querySelectorAll('.calendar_day-wrapper').forEach(dayWrapper => {
                if (dayWrapper._clickHandler) {
                    existingHandlers.set(dayWrapper, dayWrapper._clickHandler);
                }
            });
            
            // Очищаем календарь, сохраняя заголовки дней недели
            const dayHeaders = Array.from(calendarGrid.querySelectorAll('.calendar_day-header'));
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                dayWrapper.className = 'calendar_day-wrapper';
                
                // Создаем элемент для номера дня
                const dayElement = document.createElement('div');
                dayElement.setAttribute('day', '');
                dayElement.textContent = day;
                dayWrapper.appendChild(dayElement);
                
                // Создаем элемент для цены
                const priceElement = document.createElement('div');
                priceElement.setAttribute('service-price', '');
                ...
                dayWrapper.appendChild(priceElement);
                
                // Проверяем, заблокирован ли день
                if (this.isDateBlocked(dateString, this.currentMonthKey)) {
                    dayWrapper.classList.add('is-blocked');
                }
                ...
                dayWrapper._clickHandler = clickHandler;
                dayWrapper.addEventListener('click', clickHandler);
            ...
            // Инициализируем обработчики для эффекта hover
            setTimeout(() => {
     ...
     */
    getDayPrice(dateString, monthKey) {
        try {
