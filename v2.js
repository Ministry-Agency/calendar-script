...
class UnifiedCalendarManager {
    ...
    initHoverEffectHandlers() {
        try {
            ...
            // Добавляем обработчики событий mouseenter и mouseleave для каждого дня
            calendarDays.forEach(dayWrapper => {
                // Обработчик события mouseenter (наведение мыши)
                dayWrapper.addEventListener('mouseenter', () => {
                    // Если не начат выбор диапазона, ничего не делаем
                    if (this.tempSelection.isRangeConfirmed) return;
                    ...
                    if (!startDay || currentDay === startDay) return;
                    ...
                    calendarDays.forEach(day => {
                        ...
                        if (dayElement) {
                            ...
                            if (dayNum >= minDay && dayNum <= maxDay) {
                                day.style.backgroundColor = '#c4c4c4';
                                day.classList.add('hover-selected');
                            }
                        }
                    });
                    
                    console.log(`[UnifiedCalendarManager] Предварительный выбор диапазона: ${minDay}-${maxDay}`);
                });
            });
    ...
    toggleWeekendDiscountInput(show) {
        try {
            ...
            // Также ищем по id и name для надежности
            const weekendDiscountInputById = document.getElementById('Weekend-Discount-Input') || 
                                            document.querySelector('input[name="Weekend-Discount"][type="text"]');
            
            // Используем найденный элемент или первый найденный
            const inputElement = weekendDiscountInput || weekendDiscountInputById;
