...
class UnifiedCalendarManager {
     ...
     */
    isDateBlocked(dateString, monthKey) {
        try {
            // Проверка параметров
            if (!dateString || !monthKey) {
                console.error(`[UnifiedCalendarManager] Неверные параметры в isDateBlocked: дата=${dateString}, ключ=${monthKey}`);
                return false;
            }
            
            // Проверяем, есть ли данные о заблокированных датах для этого месяца
            if (!this.calendarData.blockedDates[monthKey]) {
                return false;
            }
            
            // Проверяем, есть ли эта дата в списке заблокированных
            return this.calendarData.blockedDates[monthKey].some(item => item.date === dateString);
        } catch (error) {
    ...
    /**
     * Обновление календаря при переключении месяца
     */
    updateCalendar() {
        try {
            console.log('[UnifiedCalendarManager] Обновление календаря для месяца', this.currentMonthKey);
            ...
            // Преобразуем день недели из формата JavaScript (0 - воскресенье) в формат календаря (0 - понедельник)
            // Если ваш календарь начинается с понедельника, используйте эту формулу:
            firstDayOfMonth = (firstDayOfMonth + 6) % 7;
            
            console.log(`[UnifiedCalendarManager] Первый день месяца ${month}/${year}: ${firstDay.toDateString()}, JS день недели: ${firstDay.getDay()}, календарный день недели: ${firstDayOfMonth}`);
            
            // Находим все элементы дней календаря
            const calendarDays = document.querySelectorAll('.calendar_day-wrapper');
            
            // Сначала скрываем все дни
            calendarDays.forEach(day => {
                day.classList.add('not_exist');
                day.style.display = 'none';
            });
            
            // Затем показываем только нужные дни
            for (let i = 0; i < firstDayOfMonth; i++) {
                if (i < calendarDays.length) {
                    const dayWrapper = calendarDays[i];
                    dayWrapper.classList.add('not_exist');
            ...
            for (let i = 0; i < daysInMonth; i++) {
                ...
                if (dayIndex < calendarDays.length) {
                    const dayWrapper = calendarDays[dayIndex];
                    dayWrapper.classList.remove('not_exist');
                    dayWrapper.style.display = '';
                    ...
                    // Обновляем цену
                    const dateString = this.formatDate(i + 1, month, year);
                    const price = this.getDayPrice(dateString);
                    ...
                    dayWrapper.classList.remove('hover-selected');
                    dayWrapper.style.backgroundColor = '';
                }
            }
            
            // Для оставшихся ячеек
            for (let i = firstDayOfMonth + daysInMonth; i < calendarDays.length; i++) {
                if (i < calendarDays.length) {
                    const dayWrapper = calendarDays[i];
                    dayWrapper.classList.add('not_exist');
                    dayWrapper.style.display = '';
            ...
            console.log('[UnifiedCalendarManager] Календарь успешно обновлен');
        } catch (error) {
    ...
    /**
     * Получение цены для конкретного дня
     * @param {string} dateString - Дата в формате DD.MM.YYYY
     * @returns {number} - Цена для этого дня
     */
    getDayPrice(dateString) {
        try {
            // Проверка параметров
            if (!dateString) {
            ...
            // Получаем временную метку для даты
            const [day, month, year] = dateString.split('.').map(Number);
            const timestamp = new Date(year, month - 1, day).getTime();
            
            // Проверяем, есть ли скидка для этой даты
            if (this.calendarData.dateDiscounts[timestamp]) {
                return this.calendarData.dateDiscounts[timestamp];
            }
            
            // Ищем цену для конкретной даты
            const prices = this.calendarData.basePrices[monthKey].prices;
            const priceData = prices.find(item => item.date === dateString);
            
            if (priceData) {
                return priceData.price;
            }
            
            // Если цена не найдена, возвращаем базовую цену
            return this.getBasePrice();
        } catch (error) {
    ...
    switchToPrevMonth() {
        try {
            ...
            // Если месяц стал меньше 1, переходим к декабрю предыдущего года
            if (prevMonth < 1) {
                prevMonth = 12;
                prevYear--;
            }
            
            // Форматируем новый ключ месяца
    ...
    switchToNextMonth() {
        try {
            ...
            // Если месяц стал больше 12, переходим к январю следующего года
            if (nextMonth > 12) {
                nextMonth = 1;
                nextYear++;
            }
    ...
    updateMonthTitle() {
        try {
            ...
            // Получаем текущий год и месяц из ключа
            const [year, monthNum] = this.currentMonthKey.split('-');
            
            // Получаем название месяца из обратной карты
            const monthName = REVERSE_MONTH_MAP[monthNum];
    ...
    /**
     * Обработка наведения на день календаря
     * @param {number} currentDay - Текущий день
     */
    handleDayHover(currentDay) {
        try {
    ...
    /**
     * Инициализация обработчиков для эффекта hover
     */
        ...
        try {
            ...
            calendarDays.forEach(dayWrapper => {
                ...
                if (dayElement) {
                    ...
                    dayWrapper.removeEventListener('mouseenter', dayWrapper._mouseenterHandler);
                    
                    // Создаем новый обработчик
                    dayWrapper._mouseenterHandler = () => this.handleDayHover(day);
                    
                    // Добавляем новый обработчик
                    dayWrapper.addEventListener('mouseenter', dayWrapper._mouseenterHandler);
                }
            });
            
            // Добавляем обработчик события mouseleave для календаря
            const calendar = document.querySelector('.calendar_grid');
            if (calendar) {
                // Удаляем старый обработчик, если он есть
                calendar.removeEventListener('mouseleave', calendar._mouseleaveHandler);
                
                // Создаем новый обработчик
                calendar._mouseleaveHandler = () => this.handleCalendarLeave();
                
                // Добавляем новый обработчик
                calendar.addEventListener('mouseleave', calendar._mouseleaveHandler);
            }
            
            console.log('[UnifiedCalendarManager] Обработчики для эффекта hover инициализированы');
        } catch (error) {
    ...
    /**
     * Получение элемента дня по номеру
     * @param {number} day - Номер дня
     * @returns {HTMLElement|null} - Элемент дня или null, если не найден
     */
    getDayElement(day) {
        try {
            let result = null;
            ...
            console.error('[UnifiedCalendarManager] Ошибка при получении элемента дня:', error);
            return null;
        }
    }
}
