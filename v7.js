...
class UnifiedCalendarManager {
    ...
    initMonthSwitchHandlers() {
        try {
            ...
            // Находим кнопки переключения месяцев по атрибутам
            let prevMonthButton = document.querySelector('[calendar-prev]');
            let nextMonthButton = document.querySelector('[calendar-next]');
            
            // Если кнопки не найдены по атрибутам, ищем по классам
            if (!prevMonthButton) {
                prevMonthButton = document.querySelector('.calendar_prev');
                console.log('[UnifiedCalendarManager] Кнопка предыдущего месяца найдена по классу calendar_prev');
            }
            
            if (!nextMonthButton) {
                nextMonthButton = document.querySelector('.calendar_next');
                console.log('[UnifiedCalendarManager] Кнопка следующего месяца найдена по классу calendar_next');
            }
            
            // Добавляем обработчик для кнопки предыдущего месяца
            if (prevMonthButton) {
                // Удаляем предыдущие обработчики, чтобы избежать дублирования
                prevMonthButton.removeEventListener('click', this.handlePrevMonthClick);
                
                // Сохраняем ссылку на обработчик
                this.handlePrevMonthClick = this.switchToPrevMonth.bind(this);
                
                // Добавляем новый обработчик
                prevMonthButton.addEventListener('click', this.handlePrevMonthClick);
                
                console.log('[UnifiedCalendarManager] Обработчик для кнопки предыдущего месяца добавлен');
            } else {
                console.warn('[UnifiedCalendarManager] Кнопка предыдущего месяца не найдена');
            }
            
            // Добавляем обработчик для кнопки следующего месяца
            if (nextMonthButton) {
                // Удаляем предыдущие обработчики, чтобы избежать дублирования
                nextMonthButton.removeEventListener('click', this.handleNextMonthClick);
                
                // Сохраняем ссылку на обработчик
                this.handleNextMonthClick = this.switchToNextMonth.bind(this);
                
                // Добавляем новый обработчик
                nextMonthButton.addEventListener('click', this.handleNextMonthClick);
                
                console.log('[UnifiedCalendarManager] Обработчик для кнопки следующего месяца добавлен');
            } else {
